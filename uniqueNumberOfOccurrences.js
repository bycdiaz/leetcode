...
function uniqueOccurrences(arrayOfNumbers) {
  const elementCount = Object.values(countOccurances(arrayOfNumbers));
  const allCounts = Object.values(elementCount).length;
  const uniqueCount = new Set(elementCount).size;

  return allCounts === uniqueCount;
};
